...
clearLines field
  = foldl (\currentField (line, i) -> clearLine (line - i) currentField) field 
    $ reverse $ zip linesToClear [0..]
  where
    linesToClear = map (snd . pos . head)
...
lockTetromino piece@(TetrominoState pieceType _ _) field =
  ...
  where
    ...
    newField = map (\(Cell old position) ->
...
spawnPiece piece field
  = if allowedState (piece, field)
      then Just piece
      else let movedPiece = applyAction (Move (0, 1)) piece in 
        if allowedState (movedPiece, field)
...
processAction_ (queue, piece, field) action@(Move _)
  = let newState = applyAction action piece in 
...
processActions time (queue, piece, field, lock, activeState) actions
  = if delay <= 0
    then
      ...
      in 
        if isNothing maybeSpawnedPiece
